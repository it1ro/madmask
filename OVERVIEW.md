...
1. **Управление товарами (CRUD)**
   - Модель `Product` с полями:
     ...
     - `model_url` (string) – URL GLB (относительный для `public/` или абсолютный для S3/CDN)
...
2. **3D‑превью товара**
   ...
   - Загрузка модели по `model_url`; **целевой формат MVP — GLB** (при необходимости позже — полный glTF + `.bin`).
...
4. **Производительность**
   ...
   - Модели: `public/models` (MVP) или S3 + URL в `model_url`; лимиты размера файлов — на уровне загрузки/политики хранилища.
...
- Внешний `model_url` (S3): настроить **CORS** на бакете для `GET` с origin сайта; для приватных объектов — **signed URL** с ограниченным TTL; публичные объекты — прямой URL + CORS.

## Хранение 3D-моделей и данные по средам

| Среда | Где лежат GLB | Что в БД |
|--------|----------------|----------|
| Dev / MVP | `public/models/` + относительный путь в `model_url` | путь или полный URL |
| Production | S3 (или совместимое) + опционально CDN перед бакетом | абсолютный URL или ключ + генерация URL в приложении |

- Рекомендации для S3: политика CORS, лимит размера загружаемых моделей, по возможности отдельный prefix/bucket для ассетов.
...
3. Заглушка-куб при отсутствии `model_url` или ошибке парсинга.
...
3. **Модель, Active Storage и миграции**
   - Модель `Product` с полями; `has_one_attached :cover_image`; миграции и валидации (`name`, `price`, `category`).

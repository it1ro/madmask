...
| 7.4 Микроанимации фильтров/кнопок | `app/views/products/index.html.erb`: добавлены чипы-фильтры (hover/pressed/focus, active по `params[:category]` только для разрешённых категорий); `app/views/products/_product.html.erb`: усилены hover-эффекты на CTA (“Подробнее”, “Открыть 3D превью”) с scale/glow и `prefers-reduced-motion`-совместимостью |

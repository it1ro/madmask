...
Документ отражает реализацию плана нагрузочного тестирования для madmask (Rails 8, локаль `ru` по умолчанию).

## Эталонные пути

| Сценарий | Метод и путь | Примечание |
|----------|----------------|------------|
| Главная | `GET /ru` | Избегайте `GET /` в чистых метриках ab — будет 302 на `/ru`. |
| Каталог | `GET /ru/products` | Опционально `?category=...` при фильтре. |
| Карточка товара | `GET /ru/products/:id` | Подставьте существующий `id` (см. `bin/load_test_ab`). |
| Вход Devise | `POST /ru/users/sign_in` | Не `/login`; нужны CSRF и поля `user[email]`, `user[password]`. |
| Сессия / корзина | `GET /ru/cart` | Нагрузка на cookie-сессию без отдельного маршрута `/dashboard`. |
| Админка | `GET /ru/admin` | Пока без обязательной аутентификации — не эквивалент «dashboard». |

...
- **Email-уведомление**: при успешном создании заявки отправляется письмо на `code.for.func@gmail.com` (с вложениями, если они есть). В production включена реальная доставка через SMTP (настройки через переменные окружения).
